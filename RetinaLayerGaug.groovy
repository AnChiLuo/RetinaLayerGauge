...
def main() {
    ...
    try {
        ...
        tracker.addImp(ori)
        ctx.setGapRefine(ori.getProperty("GapRefine") as boolean)
...
class PipelineContext {
    ...
    PipelineContext(ImagePlus colorImg){
        ...
        this.stainMode = colorImg.getProperty("ColorMode")
        this.MaxGapFactor = colorImg.getProperty("MaxGap") as double
    }
    ...
    void setGapRefine(boolean  GapRefine){
...
class NucleiChannelExtractor {
    ...
    NucleiChannelExtractor (ImageTracker tracker, String stainMode, Boolean batchMode){
        this.tracker = tracker
        this.stainMode = "vectors=${stainMode} hide"
    ...
    ImagePlus run(ImagePlus Imp, double ncuDia){
        ...
        IJ.log("StainedMode: ${stainMode}")
        List<ImagePlus> create =tracker.runAndReturn(
                {IJ.run(Imp, "Colour Deconvolution", stainMode)}
        ) as List<ImagePlus>
...
class NucleusExtractorOps {
    static ImagePlus EnhanceNucleiWithFilter(ImagePlus dapiImp, double Diameter, String Filter){
        ImagePlus filterImg = dapiImp.duplicate()
        if(Filter == "Morphological filter (cleaner contours; may miss faint nuclei)") {
            double MpRadius = 0.35 * Diameter
            double step = 0.5;
            MpRadius = Math.round(MpRadius/ step) * step;
            IJ.run(filterImg, "Gray Morphology", "radius=${MpRadius} type=circle operator=close")
        }
        else {
            double lowCut = Diameter * 25
            double highCut = Diameter * 1
            IJ.run(filterImg, "Bandpass Filter...", "filter_large=${lowCut} filter_small=${highCut} suppress=None tolerance=5 autoscale saturate")
            //filterImg.getProcessor().invert()
            if(Filter == "FFT + NLM denoising (best for dense tissue textures)") {
                dapiImp.getProcessor().invert()
                ImagePlus enhancedNuclei = new ImageCalculator().run("Subtract create", dapiImp, filterImg)
                IJ.run(enhancedNuclei, "Non-local Means Denoising", "sigma=15 smoothing_factor=1 auto")
...
class ExtractGapRegionStep {
    ...
    ImagePlus runII(ImagePlus Imp, double ncDia, double Tolerance, boolean GapRefine, double MaxGapF){
        IJ.run(Imp, "Fill Holes", "")
        //ImagePlus Gap = tracker.runAndReturn({Imp.cleanGap as ImagePlus})
        double minNcSize = (ncDia * 0.5) * (ncDia * 0.5) * Math.PI * 0.25
        ImagePlus cleanGap = tracker.runAndReturn({despeckle(Imp, 3)}) as ImagePlus
        if(GapRefine){
            Map<String, Object> dsResult = tracker.runAndReturn({downSampling(cleanGap, ncDia)}) as Map<String, Object>
            ImagePlus reSizeCleanGap = dsResult.resizeIp as ImagePlus
            double ScaleF = dsResult.ScaleF as double
            ImagePlus LocThk2 = thicknessMap(reSizeCleanGap, ncDia * ScaleF * Tolerance, true )
            ...
            ImageProcessor cleanGapIp = cleanGap.getProcessor()
            cleanGapIp.setValue(0)
            for(Roi roi: outlierNc){
                cleanGapIp.fill(roi)
            }
            //Interpreter.batchMode = false
            cleanGap.setProcessor("RefindNcArea", cleanGapIp )
        }
        ImagePlus LocThk = thicknessMap(cleanGap, ncDia*Tolerance, true)
        FloatProcessor LocThkIp = LocThk.getProcessor() as FloatProcessor
        LocThkIp.setThreshold(1.0,  ncDia * MaxGapF)
        ByteProcessor LocThkbp = LocThkIp.createMask()
        LocThk.setProcessor("overhangGap", LocThkbp)
        LocThk.getProcessor().invertLut()
        return LocThk

    }

    ImagePlus thicknessMap(ImagePlus rawMask, double ridgeThreshold, boolean repairOverhang = false){
        ImagePlus EDT =  tracker.runAndReturn( { IJ.run( rawMask, "Geometry to Distance Map", "threshold=128 inverse")}) as ImagePlus
        FloatProcessor EDTIp = EDT.getProcessor() as FloatProcessor
        // Remove oversized gap regions to keep subsequent processing stable.
        EDTIp.setThreshold(0, ridgeThreshold)
        ThresholdToSelection tts = new ThresholdToSelection()
        Roi GapRoi = tts.convert(EDTIp)
        EDTIp.setValue(0)
        EDTIp.fillOutside(GapRoi)
        EDT.setProcessor(EDTIp)
        ImagePlus DR =  tracker.runAndReturn ({IJ.run(EDT,"Distance Map to Distance Ridge", "")}) as ImagePlus
        ImagePlus LT =  tracker.runAndReturn ({IJ.run(DR, "Distance Ridge to Local Thickness", "")}) as ImagePlus
        ImagePlus CL =  tracker.runAndReturn ({IJ.run(LT, "Local Thickness to Cleaned-Up Local Thickness", "")}) as ImagePlus
        if(repairOverhang ){
            FloatProcessor CLIp = CL.getProcessor() as FloatProcessor
            ImageProcessor rawMaskIp = rawMask.getProcessor()
            rawMaskIp.setThreshold(255, 255)
            ThresholdToSelection maskTts = new ThresholdToSelection()
            Roi maskRoi = maskTts.convert(rawMaskIp)
            CLIp.setValue(Double.NaN)
            CLIp.fill(maskRoi)
            CL.setProcessor(CLIp)
        }
        return CL
    }
     ...
     Map<String, Object> downSampling(ImagePlus rawMask, double ncDia){
        ...
        double minncDia = 8
        if (ScaleF < 1 ){
            ImagePlus dsRawMask = rawMask.duplicate()
            ImageProcessor dsRawMaskImp = dsRawMask.getProcessor()
            dsRawMaskImp.setInterpolationMethod(ImageProcessor.NONE)
            if(minncDia > ScaleNcDia){
                ScaleF = Math.min(1.0, minncDia / ncDia)
            }
            int ScaleW = Math.round(rawMask.width * ScaleF)
            int ScaleH = Math.round(rawMask.height * ScaleF)
            dsRawMaskImp = dsRawMaskImp.resize(ScaleW, ScaleH, false)
            dsRawMask.setProcessor(dsRawMaskImp)

            return ["resizeIp": dsRawMask, "ScaleF": ScaleF ]
        }
        return ["resizeIp": rawMask, "ScaleF": 1 ]
    }



    static Roi[] removeGapBlockingNucleiStep (ImagePlus LocThk, ImagePlus RawImg ,double ncDia, boolean DEBUG_OUTLIER = false) {
        //Preparation: NaN-masked Local Thickness map
        ImagePlus MaskLocThk = LocThk.duplicate()
        FloatProcessor MaskLocThkFp = MaskLocThk.getProcessor() as FloatProcessor
        MaskLocThkFp.setThreshold(0, 0)
        ThresholdToSelection tts = new ThresholdToSelection()
        Roi AllNucleiRoi = tts.convert(MaskLocThkFp)
        MaskLocThkFp.setValue(Double.NaN)
    ...
    static double[] refinePend( double Pendx, double Pendy, double Pcx, double Pcy, double W, double H){
        double dx = Pendx - Pcx
        double dy = Pendy - Pcy
        double tx = Double.POSITIVE_INFINITY;
        double ty = Double.POSITIVE_INFINITY;

        if(Pendx < 0){tx = (0 - Pcx) / dx}
        else if(Pendx > W-1){tx = (W -1 - Pcx) / dx}
        if(Pendy < 0){ty = (0 - Pcy) / dy}
        else if(Pendy > H-1){ty = (H -1 - Pcy) / dy}

        double Finalt = Math.min(1.0, Math.min(tx, ty));
        double Px = Pcx + Finalt * dx
        double Py = Pcy + Finalt * dy
        Px = Math.max(0.0, Math.min(W - 1.0, Px));
        Py = Math.max(0.0, Math.min(H - 1.0, Py));
        return [Px, Py]
    }

    static Map<String, Object> RadiusValidator(FloatProcessor MaskLocThkFp, double [] Pstart, double r, double radius, double ncDia, double tolerance  ){
        boolean isTrim = false
        boolean outlier = false
        boolean isGCL = false
        Roi LineExt = null
        //region_locL Thickness search
        double[] Pend = [Pstart[0] + radius * Math.cos(r), Pstart[1] + radius * Math.sin(r)]
        double[] RefinePend = refinePend(Pend[0], Pend[1], Pstart[0], Pstart[1], MaskLocThkFp.width, MaskLocThkFp.height)
        if(Pend[0] != RefinePend [0] || Pend[1] != RefinePend [1]) { isTrim = true}
        double[] Line = MaskLocThkFp.getLine(Pstart[0], Pstart[1], RefinePend[0], RefinePend[1])
        ...
        for (int p = 0; p < Line.size() - 1; p++) {
            if(!Double.isNaN(Line[p])) {
                if (Line[p] > max) { max = Line[p]}
                RadiusMean += Line[p]
                nonNaN++
            }

            boolean s1 = Double.isNaN(Line[p])
            boolean s2 = Double.isNaN(Line[p + 1])
            if (s1 != s2) {
                pixelSwitch += 1
            }
        }

        //endregion_local thickness search
        ...
        else if ( pixelSwitch > 1 && isGCL){
            if( Double.isNaN(Line[0])) {
                outlier = false}
        }


        else if (pixelSwitch == 0) {
            if( !Double.isNaN(Line[0])) {
                outlier = !isTrim
            }

        }
        else if (pixelSwitch == 1) {
            if( Double.isNaN(Line[0])) {
                outlier = !isTrim
            }
    ...
    private ImagePlus despeckle(ImagePlus Imp, double minSize){
        ///Set selection parameter for ParticleAnalyzer
        int opts = ParticleAnalyzer.SHOW_MASKS
        int meas = 0
        def SizeFilter = new ParticleAnalyzer(opts, meas, new ResultsTable(),minSize, Double.POSITIVE_INFINITY, 0.0, 1.0)
        Imp.getProcessor().setThreshold(255, 255, ImageProcessor.NO_LUT_UPDATE)
        SizeFilter.analyze(Imp)
        ImagePlus cleanMask = SizeFilter.getOutputImage()
        return cleanMask
    }
}
    ...
    static List<MainChainPrepResult> prepareMainChains(List<ObjectItem> objItemList, ImagePlus rawImg){
        ...
        objItemList.withIndex().collect{objItem, index ->
            ...
            if(chainValidation == false){
                ...
                IJ.log("Secondary Skeletalization")
                ...
                if (chainValidation2nd == false){
                    IJ.log("Fail to Secondary Skeletalization")
...
class RetinaSpatialOps{
    static Map<String, ImagePlus> roiToSkeletonImp(ImagePlus sourceImp, Roi roi){
        String CLMethod = sourceImp.getProperty("CLMethod")
        ...
        if(CLMethod == "Binary Skeleton") {
            ...
            EDM CenterLineEDM = new EDM()
            ImageProcessor roiSkeletonIp = roiSkeletonImp.getProcessor()
            FloatProcessor SkeletonEDM = CenterLineEDM.makeFloatEDM(roiSkeletonIp, 0, false )
            roiSkeletonImp.setProcessor(SkeletonEDM)
            IJ.run(roiSkeletonImp, "Top Hat...", "radius=2")
            ImageProcessor topHatIp = roiSkeletonImp.getProcessor()
            ImageStatistics CLState = topHatIp.getStatistics()
            Double CLMax = CLState.max
            topHatIp.setThreshold(0.1, CLMax, ImageProcessor.RED_LUT)
...
class RefineMainChainOps{
    static List<List<Point>> refineMainChains(List<ObjectItem> objItemList, int ncuDia ) {
        ...
        objItemList.withIndex().collect{ObjectItem objItem, int Index ->
            ...
            if(!objItem.mainChain) {
                "【MainChainRefine】 Fail to get MainChain ${Index}/${objItemList.size()}"
                return null}
            ...
            return mainChain[cutLeft .. mainChain.size() - cutRight]
        }
    }
    /**
     * 建立法線路徑 (Normal Line)
...
ImagePlus loadValidImage() {
    while(true) {
        ...
        if (Double.isNaN(input.ncDia)){
            IJ.showMessage("Error" , "Please enter a valid nuclear diameter, or set it to 0 to measure later. ")
            continue
        }

        if(Double.isNaN(input.x.value) || Double.isNaN(input.y.value)){
            IJ.showMessage("Error" , "Please enter a valid scaling number. ")
            continue
        }
        ...
        ori.setProperty("PPMethod", input.PPMethod)
        ori.setProperty("CLMethod", input.CLineMethod)
        ...
        ori.setProperty("ncDia", input.ncDia)
        ori.setProperty("GapRefine", input.GapRefine)
        ori.setProperty("ColorMode", input.colorMode )
        ori.setProperty("MaxGap",input.MaxGapSize)
        return ori
    }
}
...
def runNucleusExtraction(PipelineContext ctx) {
    ...
    String stainMode = ctx.getStainMode()
    NucleiChannelExtractor nucExtractor = new NucleiChannelExtractor(tracker,stainMode, batchMode)
...
def askUserInput() {
    ...
    def openfont = new Font("SansSerif", Font.BOLD, 14)
    def darkRed = new Color(227, 23, 13)
    def Black = new Color(90, 90,90)
    def openImg = new GenericDialog("RetinaLayerGauge - Setup")
    openImg.setInsets(5,6,0)
    openImg.addFileField("Input retinal image (.tiff):", null)
    //openImg.setInsets(5,5,0)
    //openImg.addMessage("Color deconvolution mode:", openfont, darkRed)
    openImg.setInsets(5,0,0)
    openImg.addChoice("  Stain separation mode:", ["H&E", "[H&E 2]"] as String[], "H&E")
    openImg.setInsets(5,5,0)
    openImg.addMessage("Estimated nuclear diameter:", openfont, darkRed)
    openImg.setInsets(5,-120,0)
    openImg.addNumericField("", 0, 2, 8, " pixel       (Optional. Set to 0 to measure later)")
    //openImg.addToSameRow()
    //openImg.addMessage(" (Optional. Set to 0 to measure later)",new Font("SansSerif", Font.ITALIC, 12))
    openImg.setInsets(10,5,0)
    openImg.addMessage("Nuclear detection method:", openfont, darkRed)
    openImg.setInsets(-5,15,0)
    openImg.addRadioButtonGroup("", ["Morphological filter (cleaner contours; may miss faint nuclei)",  "FFT band-pass (recommended; prevents missing weak nuclei)", "FFT + NLM denoising (best for dense tissue textures)"] as String[], 3, 1, "FFT band-pass (recommended; prevents missing weak nuclei)")
    openImg.setInsets(5,5,0)
    openImg.addMessage("Gap refinement:", openfont, darkRed)
    openImg.setInsets(5,0,0)
    openImg.addNumericField("     Maximum gap size to bridge:", 2, 2, 4, " x nuclear diameter")
    openImg.setInsets(5,15,0)
    openImg.addCheckbox("Remove gap-blocking nuclei", true)
    openImg.setInsets(5,5,0)
    openImg.addMessage("Centerline detection method:", openfont, darkRed)
    openImg.setInsets(-5,15,0)
    openImg.addRadioButtonGroup("", ["Binary Skeleton", "Distance Ridge (TopHat)"] as String [], 2 ,1, "Binary Skeleton")
    openImg.setInsets(10,5,0)
    openImg.addMessage("Image scale: ", openfont, Black)
    //set scale(Pixel width, Pixel height, number unit)
    openImg.setInsets(0,0,0)
    openImg.addNumericField("Horizontal pixel size:", 1, 2, 6, "")
    //openImg.addToSameRow()
    //openImg.setInsets(0,15,0)
    //openImg.addStringField("unit:", "pixel")
    openImg.addNumericField("Vertical pixel size:", 1, 2,6,"")
    //openImg.addToSameRow()
    openImg.addStringField("Pixel size unit:", "pixel",5)
    openImg.showDialog()
    ...
    return [
            ImgPath: pathName,
            colorMode: openImg.getNextChoice(),
            ncDia: openImg.getNextNumber(),
            PPMethod : openImg.getNextRadioButton(),
            GapRefine: openImg.getNextBoolean(),
            MaxGapSize: openImg.getNextNumber(),
            CLineMethod : openImg.getNextRadioButton(),
            x : [ value: openImg.getNextNumber()],
...
class MaskGuiController {
    ...
    def buildBrushPanel(MaskBrushTool MaskBrush) {
        ...
        def fillMask = new JButton("Connect Object")
        def splitMask = new JButton(" Split Object ")
