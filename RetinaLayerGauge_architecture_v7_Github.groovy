...
class ExtractGapRegionStep {
    ...
    ImagePlus run(ImagePlus Imp, double Diameter){
        ...
        ImagePlus LocalThkEDT = tracker.runAndReturn({IJ.run(Gap_distance,"Geometry to Distance Map", "threshold=128")}) as ImagePlus
        ImageProcessor LThEDTIp = LocalThkEDT.getProcessor()
        LThEDTIp.setThreshold(0, Diameter*3)
        ThresholdToSelection filterEDT = new ThresholdToSelection()
        Roi filterGap = filterEDT.convert(LThEDTIp)
        LThEDTIp.setValue(0.0)
        LThEDTIp.fillOutside(filterGap)
        ImagePlus LocalThkDR = tracker.runAndReturn({IJ.run(LocalThkEDT, "Distance Map to Distance Ridge", "")}) as ImagePlus
        ImagePlus LocThk = tracker.runAndReturn({IJ.run(LocalThkDR,"Distance Ridge to Local Thickness", "")}) as ImagePlus
